defmodule ArtemisWeb.WikiPageCommentControllerTest do
  ...
  describe "create comment" do
    ...
    test "redirects to show when data is valid", %{conn: conn, parent: parent} do
      conn = post(conn, Routes.wiki_page_comment_path(conn, :create, parent), comment: @create_attrs)
    ...
    test "renders errors when data is invalid", %{conn: conn, parent: parent} do
      conn = post(conn, Routes.wiki_page_comment_path(conn, :create, parent), comment: @invalid_attrs)
  ...
  describe "edit comment" do
    ...
    test "renders form for editing chosen comment", %{conn: conn, parent: parent, record: record} do
      conn = get(conn, Routes.wiki_page_comment_path(conn, :edit, parent, record))
  ...
  describe "update comment" do
    ...
    test "redirects when data is valid", %{conn: conn, parent: parent, record: record} do
      conn = put(conn, Routes.wiki_page_comment_path(conn, :update, parent, record), comment: @update_attrs)
      assert redirected_to(conn) == Routes.wiki_page_path(conn, :show, parent)

      conn = get(conn, Routes.wiki_page_path(conn, :show, parent))
    ...
    test "renders errors when data is invalid", %{conn: conn, parent: parent, record: record} do
      conn = put(conn, Routes.wiki_page_comment_path(conn, :update, parent, record), comment: @invalid_attrs)
      assert html_response(conn, 200) =~ "Edit Comment"
      assert html_response(conn, 200) =~ "Error"
    end
  end
  ...
  describe "delete comment" do
    ...
    test "deletes chosen comment", %{conn: conn, parent: parent, record: record} do
      conn = delete(conn, Routes.wiki_page_comment_path(conn, :delete, parent, record))
      assert redirected_to(conn) == Routes.wiki_page_path(conn, :show, parent)
    end
  end
  ...
  defp create_record(_) do
    parent = insert(:wiki_page)
    record = insert(:comment, wiki_pages: [parent])
