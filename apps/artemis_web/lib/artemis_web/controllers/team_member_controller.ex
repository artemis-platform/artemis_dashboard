defmodule ArtemisWeb.TeamMemberController do
  ...
  def index(conn, %{"team_id" => team_template_id}) do
    redirect(conn, to: Routes.team_path(conn, :show, team_template_id))
  end

  def new(conn, %{"team_id" => team_template_id}) do
    authorize(conn, "user-teams:create", fn ->
      user = current_user(conn)
      team_template = GetTeam.call!(team_template_id, user)
      team_user = %UserTeam{team_template_id: team_template_id}
  ...
  def create(conn, %{"team_user" => params, "team_id" => team_template_id}) do
    authorize(conn, "user-teams:create", fn ->
      user = current_user(conn)
      team_template = GetTeam.call!(team_template_id, user)
      ...
      case CreateUserTeam.call(params, user) do
        {:ok, _team_user} ->
          ...
          |> redirect(to: Routes.team_path(conn, :show, team_template_id))
  ...
  def edit(conn, %{"team_id" => team_template_id, "id" => id}) do
  ...
  def update(conn, %{"id" => id, "team_id" => team_template_id, "team_user" => params}) do
    authorize(conn, "user-teams:update", fn ->
      user = current_user(conn)
      team_template = GetTeam.call!(team_template_id, user)
  ...
  def delete(conn, %{"team_id" => team_template_id, "id" => id} = params) do
    authorize(conn, "user-teams:delete", fn ->
      {:ok, _team_user} = DeleteUserTeam.call(id, params, current_user(conn))

      conn
      |> put_flash(:info, "Team User deleted successfully.")
      |> redirect(to: Routes.team_path(conn, :show, team_template_id))
    end)
  end
end
